function initAll(clientName) {
  ...
  socket.on('connect', function () {
    // Send hello event right after connect:
    socket.emit('hello', { name: clientName });
  });
  ...
  function updateGameState(gameState) {
    ...
    document.getElementById('playerCount').innerHTML =
      String(playerCount) + ' tripogordo' + (playerCount > 1 ? 's' : '') + ' en la oficina';
    var scores = '';
